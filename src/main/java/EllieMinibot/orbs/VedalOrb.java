...
public class VedalOrb extends AbstractOrb {
    private static final OrbStrings orbString;
    private static final Texture myTex = TexLoader.getTexture(makeImagePath("/orbs/VedalOrb.png"));

    public VedalOrb() {
        this.ID = makeID("VedalOrb");
        this.img = myTex;
    ...
    public void onEvoke() {
        ...
        for(int j = this.evokeAmount - 1; j >= 0; j--) {
            for (int i = playerHand.size() - 1; i >= 0; i--) {
                AbstractCard card = playerHand.get(i);
                if (!card.upgraded && ((card.color != AbstractCard.CardColor.CURSE) && (card.type != AbstractCard.CardType.STATUS))) {
    ...
    public void onStartOfTurn() {
        ...
        for(int j = this.passiveAmount - 1; j >= 0; j--) {
    ...
    static {
        orbString = CardCrawlGame.languagePack.getOrbString("VedalOrb");
    }
}
