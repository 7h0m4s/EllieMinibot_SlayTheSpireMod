...
public class EllieOrb extends AbstractOrb {
    ...
    private static final Texture myTex = TexLoader.getTexture(makeImagePath("/orbs/EllieOrb.png"));

    public EllieOrb() {
        this.ID = makeID("EllieOrb");
        this.img = myTex;
    ...
    public void onEvoke() {
        ...
        att(new MakeTempCardInHandAction(new BugFactCard(), this.evokeAmount));
    }

    public void onStartOfTurn() {
        att(new VFXAction(new DarkOrbActivateEffect(hb.cX, hb.cY), 0.1F));
        att(new SFXAction("MONSTER_SLIME_ATTACK", 0.5F));
        att(new MakeTempCardInHandAction(new BugFactCard(), this.passiveAmount));
    }
    ...
    static {
        orbString = CardCrawlGame.languagePack.getOrbString("EllieOrb");
    }
}
